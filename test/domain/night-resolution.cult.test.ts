...
describe('resolveCultNight', () => {
  ...
  it('converts a default-chance role (e.g. Villager) on a successful roll', () => {
    ...
    const events = resolveCultNight(
      [cultist, villager],
      initialNightState(),
      baseCtx([cultist, villager], 3, () => 0),
    );
    ...
    expect(
      events.some((e) => e.type === 'PlayerConvertedToCult' && e.playerId === villager.id),
    ).toBe(true);
  });
  ...
  it('picks the most-voted target among several acting cultists', () => {
    ...
    resolveCultNight(
      [c1, c2, popular, unpopular],
      initialNightState(),
      baseCtx([c1, c2, popular, unpopular], 3, () => 0),
    );
  ...
  it('lets the Hunter kill the newbie when conversion fails but the kill-roll succeeds', () => {
    ...
    const events = resolveCultNight(
      [cultist, hunter],
      initialNightState(),
      baseCtx([cultist, hunter], 3, random),
    );
  ...
  it('kills the newbie visiting a wolf who stayed home, but not one who went out hunting', () => {
    ...
    const eventsStayedHome = resolveCultNight(
      [cultist1, wolf1],
      stayHomeState,
      baseCtx([cultist1, wolf1]),
    );
    expect(cultist1.isDead).toBe(true);
    expect(eventsStayedHome.some((e) => e.type === 'PlayerDied' && e.method === 'VisitWolf')).toBe(
      true,
    );
  ...
  it('always fails to convert an Arsonist who is out actively dousing/sparking (no attempt at all)', () => {
    ...
    const events = resolveCultNight(
      [cultist, arsonist],
      initialNightState(),
      baseCtx([cultist, arsonist], 3, () => 0),
    );

    expect(arsonist.role).toBe(ROLE_BIT.Arsonist);
    expect(
      events.some((e) => e.type === 'CultConversionFailed' || e.type === 'PlayerConvertedToCult'),
    ).toBe(false);
  });
